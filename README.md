...
Each key in the KV store represents the vanity package name. For example, if you want to point `go.joshhogle.com/toolbox` to `github.com/josh-hogle/go-toolbox`, the key should be `go.joshhogle.com/toolbox`.
...
`source` is required and represents the source path to the repository (eg: `github.com/josh-hogle/go-toolbox`).
...
If you use plaintext, the value should simply be the path to the source repository. In the example above, the value would be `github.com/josh-hogle/go-toolbox`. Values for `vcs` and `defaultBranch` in this case are set to the default values.
...
If you have questions about this project, find a bug or wish to submit a feature request, please [submit an issue](https://github.com/josh-hogle/go-vanity-cf-worker/issues).
